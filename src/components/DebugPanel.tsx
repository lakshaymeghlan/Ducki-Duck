...
interface DebugPanelProps {
  /** Triggers the duck's reply (a single `quack.`). */
  onExplain: () => void;
  bugCount: number;
}
...
export function DebugPanel({ onExplain, bugCount }: DebugPanelProps) {
  ...
  return (
    <Card
      ...
      subtitle="Say it out loud. The duck listens and contributes nothing. That's the point."
    >
      <form onSubmit={submit} className="flex flex-col gap-3">
        <label htmlFor="debug-input" className="visually-hidden">
          Tell the duck what&apos;s broken
        </label>
        <textarea
          id="debug-input"
          value={text}
          ...
          rows={3}
          placeholder="Tell the duck what's broken…"
      ...
      {bugCount > 0 && (
        <p className="mt-3 text-sm text-ink-soft">
          ...
          {bugCount === 1 ? "bug" : "bugs"} to the duck today. It still has no
          notes.
        </p>
      )}
    </Card>
